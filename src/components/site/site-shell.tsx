...
export function SiteShell({
  ...
  locale: "de" | "en";
  user?: SiteUser;
  publicConfig: PublicShellConfig;
  children: ReactNode;
}) {
  ...
  const cartTotalRappen = items.reduce(
  ...
  const labels = de
    ? {
        ...
        privacy: "Datenschutz",
        terms: "AGB",
        welcome: "Willkommen bei SaltNPepper",
      }
    : {
        home: "Home",
        menu: "Menu",
        ...
        privacy: "Privacy",
        terms: "Terms",
        welcome: "Welcome to SaltNPepper",
      };
  const navLinks = [
  ...
  return (
    <div className="flex min-h-dvh flex-col" lang={locale}>
      <a
        href="#main-content"
        className="sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:block focus:rounded-xl focus:bg-primary focus:px-5 focus:py-3 focus:font-bold focus:text-white"
      >
          ...
          <header>
            <div className="flex min-h-16 items-center justify-between gap-3 px-4 sm:px-5">
              <Link
                ...
                aria-label={de ? "SaltNPepper Startseite" : "SaltNPepper home"}
              >
                <BrandLogo
                  priority
              ...
              >
                {navLinks.map((link) => (
                  <Link
                    ...
                    className={`inline-flex min-h-11 items-center rounded-xl px-4 text-sm font-bold transition-colors ${isActive(link.href) ? "bg-primary text-white" : "text-foreground hover:bg-surface-warm"}`}
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                {user && ["CUSTOMER", "OWNER"].includes(user.role ?? "") && (
                  <CustomerNotifications locale={locale} userId={user.id} />
                )}
                <Link
                  href={switchLangPath}
                  hrefLang={other}
                ...
                <Link
                  href={`/${locale}/cart`}
                  className="relative inline-flex min-h-11 items-center gap-2 rounded-xl bg-primary px-3.5 text-sm font-bold text-white transition-colors hover:bg-primary-light"
                  aria-label={`${labels.cart}: ${count}`}
                >
                  <ShoppingBag className="h-4 w-4" aria-hidden="true" />
                  <span className="hidden lg:inline">{labels.cart}</span>
                  {count > 0 && (
                    <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-secondary px-1 text-[11px] text-white">
                      {count}
                    </span>
                  )}
                </Link>
                ...
                >
                  <button
                    ...
                    className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-border bg-surface text-primary hover:border-secondary"
                    aria-expanded={userDropdownOpen}
                    aria-haspopup="menu"
                    aria-label={de ? "Benutzermenü" : "User menu"}
                  >
                    {user?.image ? (
                      <Image
                        src={user.image}
                  ...
                  {userDropdownOpen && (
                    <div
                      role="menu"
                      className="absolute right-0 top-[3.25rem] z-50 w-72 rounded-2xl border border-border bg-surface p-2 shadow-2xl"
                    >
                      {user ? (
                        <>
                          ...
                          <div className="mt-2 space-y-1">
                            <Link
                              ...
                              className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-bold hover:bg-surface-warm"
                            >
                              <ShoppingBag
                          ...
                          <button
                            ...
                            className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-bold text-destructive hover:bg-destructive/10"
                          >
                        ...
                        <div className="p-2">
                          ...
                          <p className="mt-1 text-xs leading-5 text-muted">
                            {de
                              ? "Melden Sie sich an, um Bestellungen zu verfolgen."
                          ...
                          <div className="mt-4 grid gap-2">
                            <Link
                              ...
                              href={`/${locale}/login`}
                              onClick={() => setUserDropdownOpen(false)}
                              className="inline-flex min-h-11 items-center justify-center rounded-xl bg-primary font-bold text-white"
                            >
                              {labels.login}
                            </Link>
                            <Link
                              ...
                              onClick={() => setUserDropdownOpen(false)}
                              className="inline-flex min-h-11 items-center justify-center rounded-xl border border-primary/25 font-bold text-primary"
                            >
                              {labels.register}
                            </Link>
                          </div>
                ...
                <button
                  ...
                  aria-label={de ? "Navigation öffnen" : "Open navigation"}
                >
                  {mobileMenuOpen ? (
                ...
                <div className="grid gap-1">
                  {navLinks.map((link) => (
                    <Link
                      ...
                      aria-current={isActive(link.href) ? "page" : undefined}
                      className={`flex min-h-12 items-center rounded-xl px-4 font-bold ${isActive(link.href) ? "bg-primary text-white" : "hover:bg-surface-warm"}`}
                    >
                      {link.label}
                    </Link>
                  ))}
      ...
      <main
        id="main-content"
        tabIndex={-1}
        className={`flex-1 focus:outline-none ${announcement ? "pt-[8.25rem] sm:pt-[7.75rem]" : "pt-[5.5rem]"}`}
      >
        {children}
      </main>
      ...
      <footer className="border-t border-border bg-surface">
        <div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-16">
          <div className="grid gap-10 md:grid-cols-[1.25fr_0.75fr_1fr]">
            <div>
              ...
              <p className="mt-5 max-w-sm text-sm leading-6 text-muted">
                {de
                  ? "Frisch. Lokal. Einfach in Oberglatt."
                  : "Fresh. Local. Simple in Oberglatt."}
              </p>
              <div className="mt-5">
                <SocialLinks
                  facebookUrl={publicConfig.facebookUrl}
            ...
            <div>
              ...
              <address className="mt-4 space-y-3 text-sm not-italic text-muted">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex min-h-11 items-start gap-3 hover:text-primary"
                >
                  <MapPin
                ...
                <a
                  ...
                  className="flex min-h-11 items-center gap-3 hover:text-primary"
                >
                  <Phone
                ...
                <a
                  ...
                  className="flex min-h-11 items-center gap-3 break-all hover:text-primary"
                >
                  <Mail
          ...
          <div className="mt-12 flex flex-col gap-4 border-t border-border pt-7 text-xs text-muted sm:flex-row sm:items-center sm:justify-between">
            <p>
              © {new Date().getFullYear()} SaltNPepper. {labels.rights}
            </p>
      ...
      <a
        ...
        aria-label={
          de
            ? "SaltNPepper auf WhatsApp kontaktieren"
            : "Contact SaltNPepper on WhatsApp"
        }
      ...
      {count > 0 && (
        <div className="fixed bottom-4 left-1/2 z-40 w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 sm:hidden">
          <button
            ...
            className="flex min-h-14 w-full items-center justify-between rounded-2xl bg-primary px-5 text-white shadow-2xl"
            aria-label={
              de
                ? `Warenkorb ansehen, ${count} Artikel`
