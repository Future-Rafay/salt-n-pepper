...
export async function generateMetadata({
  ...
  return localizedMetadata(
    ...
    "",
    {
      de: "SaltNPepper Restaurant Oberglatt",
      en: "SaltNPepper Restaurant Oberglatt",
    },
    {
...
export default async function HomePage({
  ...
  const de = locale === "de";
  const [{ brand, fulfillment, hours }, categories, latestPosts] =
    await Promise.all([
      getPublicConfig(),
  ...
  const orderingReady =
    products.length > 0 &&
    (fulfillment.deliveryEnabled || fulfillment.pickupEnabled);
  const heroTitle =
    (de ? brand.heroTitleDe : brand.heroTitleEn) || copy.heroTitle;
  const heroSubtitle =
    (de ? brand.heroSubtitleDe : brand.heroSubtitleEn) || copy.heroSubtitle;
  ...
  return (
    <div className="overflow-hidden pb-20 sm:pb-28">
      <section className="mx-auto grid min-h-[calc(100dvh-5rem)] max-w-[1440px] items-stretch lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="relative min-h-[58dvh] overflow-hidden bg-primary lg:min-h-full">
          <Image
            ...
            alt={
              de
                ? "Editorial gedeckter Restauranttisch"
                : "Editorial restaurant table setting"
            }
            fill
            priority
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-4 text-white sm:bottom-10 sm:left-10 sm:right-10">
            ...
            <span className="font-display text-5xl text-secondary sm:text-7xl">
              SNP
            </span>
          </div>
        </div>
      ...
      <section
        aria-label={de ? "Kontaktinformationen" : "Contact information"}
      ...
      >
        <div className="mx-auto grid max-w-7xl divide-y divide-white/15 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex min-h-20 items-center gap-3 px-5 py-4 transition-colors hover:bg-white/5 sm:px-8"
          >
            <MapPin
              className="h-5 w-5 shrink-0 text-secondary-light"
              aria-hidden="true"
            />
            <span className="text-sm font-medium">
              Allmendstrasse 18, 8154 Oberglatt
            </span>
          </a>
          <a
            href={`tel:${restaurantContent.phone.replace(/\s/g, "")}`}
          ...
          >
            ...
            />
            <span className="text-sm font-medium">
              {restaurantContent.phone}
            </span>
          </a>
          <a
            href={`mailto:${restaurantContent.email}`}
            className="flex min-h-20 items-center gap-3 px-5 py-4 transition-colors hover:bg-white/5 sm:px-8"
          >
            <Mail
              className="h-5 w-5 shrink-0 text-secondary-light"
        ...
        <div className="flex flex-col justify-between gap-5 border-b border-primary/15 pb-8 md:flex-row md:items-end">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-secondary">
              {de ? "Von der Küche" : "From the kitchen"}
            </p>
          ...
          {featured.length > 0 && (
            <Link
              href={`/${locale}/menu`}
              className="inline-flex min-h-11 items-center gap-2 font-bold text-primary hover:text-secondary"
            >
        ...
        {featured.length > 0 ? (
            ...
            {featured.map((product) => (
              ...
              >
                <div className="relative aspect-[4/3] bg-surface-warm">
                  <Image
                    src={product.imageKey || restaurantContent.heroImage}
                    alt={product.name}
                    fill
                ...
                <div className="p-6">
                  ...
                  <h3 className="mt-2 font-display text-2xl tracking-[-0.03em] text-primary">
                    {product.name}
                  </h3>
                  {product.description && (
          ...
          <div className="mt-10 grid overflow-hidden rounded-card border border-border bg-surface lg:grid-cols-[0.9fr_1.1fr]">
            <div className="relative min-h-72 lg:min-h-[420px]">
              <Image
                src="/images/editorial/restaurant-atmosphere.jpg"
                alt={
                  de
                    ? "Editoriale Restaurantatmosphäre"
                    : "Editorial restaurant atmosphere"
                }
                fill
            ...
            <div className="flex flex-col justify-center p-8 sm:p-12 lg:p-16">
              <UtensilsCrossed
                ...
                aria-hidden="true"
              />
              <h3 className="mt-6 font-display text-3xl tracking-[-0.035em] text-primary sm:text-4xl">
                {de
                  ? "Echte Gerichte statt Platzhalter."
                  : "Real dishes, not placeholders."}
              </h3>
              <p className="mt-4 max-w-xl text-base leading-7 text-muted">
                {de
                  ? "Sobald das SaltNPepper Menü bestätigt ist, finden Sie hier Namen, Preise, Varianten und Bestellmöglichkeiten."
                  : "As soon as the SaltNPepper menu is confirmed, you will find real names, prices, options, and ordering here."}
              </p>
              <Link
                href={`/${locale}/contact`}
                className="mt-8 inline-flex min-h-11 w-fit items-center gap-2 font-bold text-primary hover:text-secondary"
              >
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-secondary-contrast">
              SaltNPepper · Oberglatt
            </p>
            <h2 className="mt-4 max-w-3xl font-display text-4xl leading-[0.95] tracking-[-0.04em] sm:text-6xl">
              {copy.aboutTitle}
            </h2>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link
                href={`/${locale}/about`}
                className="inline-flex min-h-12 items-center justify-center rounded-xl bg-secondary px-6 font-bold text-white hover:bg-secondary-light"
              >
                {de ? "Über uns" : "About us"}
              </Link>
              <Link
                href={`/${locale}/contact`}
                className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/25 px-6 font-bold text-white hover:bg-white/5"
              >
                {de ? "Kontakt" : "Contact"}
              </Link>
            </div>
          ...
          <div className="relative aspect-[4/5] overflow-hidden rounded-card border border-white/15">
            <Image
              src="/images/editorial/food-preparation.jpg"
              alt={
                de
                  ? "Editorial gedeckter Tisch mit frischen Speisen"
                  : "Editorial table with freshly prepared food"
              }
              fill
      ...
      {latestPosts.length > 0 && (
        ...
        >
          <div className="flex flex-col justify-between gap-5 border-b border-primary/15 pb-8 md:flex-row md:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-secondary">
                SweetNSavour · SaltNPepper
              </p>
              <h2
                id="latest-blog-heading"
                className="mt-3 max-w-3xl font-display text-4xl leading-none tracking-[-0.04em] text-primary sm:text-6xl"
              >
                {de
                  ? "Frische Ideen aus dem Blog."
                  : "Fresh ideas from the blog."}
              </h2>
              <p className="mt-4 max-w-2xl leading-7 text-muted">
                {de
                  ? "Food-Guides, Ernährungstipps und Inspiration für Ihren Alltag."
                  : "Food guides, nutrition tips, and practical inspiration for everyday life."}
              </p>
            </div>
            <Link
              href={`/${locale}/blog`}
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <article className="rounded-card border border-border bg-surface p-7">
            ...
            <h2 className="mt-5 font-display text-2xl text-primary">
              {de ? "Online bestellen" : "Order online"}
            </h2>
            <p className="mt-3 text-sm leading-6 text-muted">
              {orderingReady
                ? de
                  ? "Wählen Sie Ihre Gerichte und die verfügbare Abhol- oder Lieferoption."
                  : "Choose your dishes and an available pickup or delivery option."
                : de
                  ? "Die Online-Bestellung wird freigeschaltet, sobald Menü und Servicezeiten bestätigt sind."
                  : "Online ordering will open once the menu and service hours are confirmed."}
            </p>
          </article>
          <article className="rounded-card border border-border bg-surface p-7">
            ...
            <p className="mt-3 text-sm leading-6 text-muted">
              {hours.length > 0
                ? de
                  ? "Die aktuellen Servicezeiten finden Sie auf unserer Kontaktseite."
                  : "Current service hours are available on our contact page."
                : de
                  ? "Bestätigte Öffnungszeiten werden hier veröffentlicht."
          ...
          <article className="rounded-card border border-border bg-surface p-7">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-5 inline-flex min-h-11 items-center gap-2 font-bold text-primary hover:text-secondary"
            >
              {de ? "Route öffnen" : "Open directions"}
              <ArrowUpRight className="h-5 w-5" aria-hidden="true" />
            </a>
