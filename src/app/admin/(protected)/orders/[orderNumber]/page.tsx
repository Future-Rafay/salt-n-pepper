...
export default async function OrderDetailPage({ params, searchParams }: { params: Promise<{ orderNumber: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const paidStripe = order.payment?.provider === "STRIPE" && ["PAID", "PARTIALLY_REFUNDED"].includes(order.payment.status);
  return <AdminPage title={order.orderNumber} description={`${order.fulfillmentType.replaceAll("_", " ")} · placed ${new Date(order.createdAt).toLocaleString("en-CH", { timeZone: "Europe/Zurich" })}`}>
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <Card className="lg:col-span-2"><h2 className="font-display text-xl font-semibold">Order</h2>
        ...
        <ul className="mt-5 space-y-3 border-t pt-4">{order.items.map((item) => <li key={item.id} className="flex justify-between gap-4"><span>{item.quantity}× {item.productNameEnSnapshot}<small className="block text-muted">{item.variantNameEnSnapshot}{item.options.length ? ` · ${item.options.map((option) => option.nameEnSnapshot).join(", ")}` : ""}</small></span><strong>{formatMoney(item.lineSubtotalRappen, "en")}</strong></li>)}</ul>
        <dl className="mt-5 ml-auto max-w-xs space-y-2 border-t pt-4 text-sm"><div className="flex justify-between"><dt>Subtotal</dt><dd>{formatMoney(order.subtotalRappen, "en")}</dd></div><div className="flex justify-between"><dt>Discount</dt><dd>−{formatMoney(order.discountRappen, "en")}</dd></div>{order.fulfillmentType === "DELIVERY" && <div className="flex justify-between"><dt>Delivery</dt><dd>{order.deliveryFeeRappen ? formatMoney(order.deliveryFeeRappen, "en") : "Free"}</dd></div>}<div className="flex justify-between text-base font-bold"><dt>Total</dt><dd>{formatMoney(order.totalRappen, "en")}</dd></div></dl>
      </Card>
    ...
    <section className="mt-8 rounded-[var(--radius-card)] border border-destructive/40 bg-surface p-5" aria-labelledby="danger-zone"><h2 id="danger-zone" className="font-display text-xl font-semibold text-destructive">Danger zone</h2><p className="mt-1 text-sm text-muted">Cancellation and refunds are intentionally separated from routine status actions.</p><div className="mt-5 flex flex-wrap gap-3">{canCancel && !paidStripe && <CancelOrderDialog orderNumber={order.orderNumber} returnTo={returnTo} />}{paidStripe && order.remainingRefundableRappen > 0 && user?.role === "OWNER" && <RefundDialog orderNumber={order.orderNumber} remainingRappen={order.remainingRefundableRappen} refundKey={randomUUID()} returnTo={returnTo} />}{canCancel && paidStripe && order.remainingRefundableRappen > 0 && <RefundDialog orderNumber={order.orderNumber} remainingRappen={order.remainingRefundableRappen} refundKey={randomUUID()} returnTo={returnTo} fullRefundOnly />}</div>{order.payment?.refunds.length ? <div className="mt-6 border-t pt-4"><h3 className="font-semibold">Refunds</h3><ul className="mt-3 space-y-2 text-sm">{order.payment.refunds.map((refund) => <li key={refund.id}>{formatMoney(refund.amountRappen, "en")} · {refund.status} · {refund.reason} · by {refund.requestedBy?.name || refund.requestedBy?.email || "system"}</li>)}</ul></div> : null}</section>
  </AdminPage>;
}
