  ...
  return <AdminPage title={order.orderNumber} description={`${order.fulfillmentType.replaceAll("_", " ")} · placed ${new Date(order.createdAt).toLocaleString("en-CH", { timeZone: "Europe/Zurich" })}`}>
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <Card className="lg:col-span-2"><h2 className="font-display text-xl font-semibold">Order</h2>
        ...
        <dl className="mt-5 ml-auto max-w-xs space-y-2 border-t pt-4 text-sm"><div className="flex justify-between"><dt>Subtotal</dt><dd>{formatChf(order.subtotalRappen, "en")}</dd></div><div className="flex justify-between"><dt>Discount</dt><dd>−{formatChf(order.discountRappen, "en")}</dd></div>{order.fulfillmentType === "DELIVERY" && <div className="flex justify-between"><dt>Delivery</dt><dd>{order.deliveryFeeRappen ? formatChf(order.deliveryFeeRappen, "en") : "Free"}</dd></div>}<div className="flex justify-between text-base font-bold"><dt>Total</dt><dd>{formatChf(order.totalRappen, "en")}</dd></div></dl>
      </Card>
